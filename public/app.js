...
async function copyToPrevDay() {
  ...
  const doCopy = () => {
    ...
    // Solo copiamos los tags, NO el texto, para evitar traer contenido eliminado
    const prevDataCopy = {};
    CATS.forEach(cat => {
      ...
      if (entry) {
        prevDataCopy[cat.id] = {
          tags: Array.isArray(entry.tags) ? [...entry.tags] : [],
          text: '' // No copiamos el texto, solo los tags
        };
      } else {
  ...
  try {
    ...
    DAYS.forEach(day => {
      ...
      CATS.forEach(cat => {
        ...
        nextEntry[cat.id] = {
          tags: Array.isArray(entry?.tags) ? [...entry.tags] : [],
          text: '' // No copiamos el texto, solo los tags para evitar traer contenido eliminado
        };
      });
